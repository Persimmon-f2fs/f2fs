				 ...
				 long nr_to_write, enum iostat_type io_type)
{
	...
	while ((nr_pages = pagevec_lookup_tag(&pvec, mapping, &index,
		...
		for (i = 0; i < nr_pages; ++i) {
			...
			f2fs_wait_on_page_writeback(page, META_MAPPED, true,
						    true);
...
stop:
	...
	// truncate_inode_pages(mapping, le32_to_cpu(F2FS_RAW_SUPER(sbi)->last_ssa_blkaddr) + 1);

	// sync possible cp / physical data
	return nwritten;
}

long f2fs_sync_meta_chunk_pages(struct f2fs_sb_info *sbi, enum page_type type,
				long nr_to_write, enum iostat_type io_type)
{
	...
	while ((nr_pages = pagevec_lookup_tag(&pvec, mapping, &index,
		...
		for (i = 0; i < nr_pages; ++i) {
			...
			f2fs_wait_on_page_writeback(page, META_CHUNK, true,
						    true);

			if (!clear_page_dirty_for_io(page))
				goto continue_unlock;

			if (write_chunk_page(page, &wbc)) {
				unlock_page(page);
				break;
			}
...
retry:
	if (!e)
		new = f2fs_kmem_cache_alloc(ino_entry_slab, GFP_NOFS, true,
					    NULL);
...
int get_checkpoint_version(struct f2fs_sb_info *sbi, block_t cp_addr,
			   ...
			   struct page **cp_page, unsigned long long *version)
{
	size_t crc_offset = 0;
	__u32 crc;
...
int do_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	ckpt->cp_pack_start_meta_bat =
		cpu_to_le32(1 + cp_payload_blks + orphan_blocks);
	ckpt->cp_pack_start_meta_bit =
		cpu_to_le32(le32_to_cpu(ckpt->cp_pack_start_meta_bat) +
			    F2FS_BAT_BLOCKS(sbi));
	ckpt->cp_pack_start_meta_bitmap =
		cpu_to_le32(cpu_to_le32(ckpt->cp_pack_start_meta_bit) +
			    F2FS_BIT_BLOCKS(sbi));
	ckpt->cp_pack_start_sum = cpu_to_le32(
		1 + cp_payload_blks + orphan_blocks + F2FS_BAT_BLOCKS(sbi) +
	...
	*((__le32 *)((unsigned char *)ckpt +
...
#ifdef CONFIG_BLK_DEV_ZONED
	devi = f2fs_target_device_index(
		sbi, le32_to_cpu(F2FS_RAW_SUPER(sbi)->cp_blkaddr));
	if (f2fs_blkz_is_seq(sbi, devi,
			     le32_to_cpu(F2FS_RAW_SUPER(sbi)->cp_blkaddr))) {
		err = zoned_cp_next_start(
			sbi, &start_blk,
	...
	kbytes_written +=
		(f2fs_get_sectors_written(sbi) - sbi->sectors_written_start) >>
		1;
...
int f2fs_write_checkpoint(struct f2fs_sb_info *sbi, struct cp_control *cpc)
{
	...
	if (err) {
		f2fs_err(
			sbi,
			"f2fs_flush_nat_entries failed err:%d, stop checkpoint",
			err);
...
int f2fs_start_ckpt_thread(struct f2fs_sb_info *sbi)
{
	...
	cprc->f2fs_issue_ckpt =
		kthread_run(issue_checkpoint_thread, sbi, "f2fs_ckpt-%u:%u",
			    MAJOR(dev), MINOR(dev));
